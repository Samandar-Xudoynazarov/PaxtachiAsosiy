...
const FarmersPage: React.FC = () => {
  ...
  const { data, loading, error, page, totalPages, search, setSearch, setPage, refetch } =
    useCrud<Farmer>({ fetchFn: farmersApi.getAll })
  ...
  useEffect(() => {
    specializationsApi.getSummary().then(res => {
      ...
      setSpecData(raw.map(r => ({
        ...r,
        percentage: total > 0 ? Math.round((r.farmersCount / total) * 100) : 0,
      })))
    }).catch(() => {})
  }, [])

  const chartData = specData.map((d, i) => ({
    name: d.name,
  ...
  const openAdd = () => {
    setForm({ name: '', stir: '', farmType: '', rightType: '', totalArea: '' })
    setAddModal(true)
  }
  ...
  const columns: Column<Farmer>[] = [
    { key: 'id', title: '#', width: '60px', render: (_, __, idx) => String((page * 10) + (idx as number) + 1) },
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Fermerlar"
        subtitle="Barcha fermerlar ro'yxati va boshqaruvi"
        actions={
          <>
        ...
        <div className="card mb-4">
          ...
          <div className="flex flex-col lg:flex-row items-start gap-6">
            ...
            <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-2 w-full">
              {specData.map((item, i) => (
                <div key={item.id} className="flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <div className="flex-1 bg-slate-200 rounded-full h-1">
                        <div
                          ...
                          style={{ width: `${item.percentage ?? 0}%`, backgroundColor: COLORS[i % COLORS.length] }}
                        />
                      </div>
      ...
      <div className="card p-0 overflow-hidden">
        <div className="p-4 border-b border-slate-100">
          <SearchBar value={search} onChange={setSearch} placeholder="Fermer qidirish…" />
        </div>
        <Table<Farmer>
          columns={columns}
          data={data}
      ...
      <Drawer
        ...
        subtitle={`INN/STIR: ${detailRow?.stir || '—'}`}
      >
      ...
      <Modal isOpen={!!deleteRow} onClose={closeAll} title="O'chirishni tasdiqlash" size="sm">
        <p className="text-slate-600 mb-6">
          <strong>{deleteRow?.name}</strong> fermerini o'chirishni xohlaysizmi?
        </p>
